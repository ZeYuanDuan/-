...
// ! 記得改回使用 AWS Secrets Manager 取得密碼
...
async function initializePool(): Promise<void> {
  ...
  pool = mysql.createPool({
    ...
    user: process.env.DB_USER, // ! 先暫時用環境變數繞過 AWS Secrets Manager，之後改用 secret
    password: process.env.DB_PASSWORD, // ! 先暫時用環境變數繞過 AWS Secrets Manager，之後改用 secret
